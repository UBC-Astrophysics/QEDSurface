...
from ximpol.config.herx1_pulsar11on import pol_degree_spline, \
    pol_angle_spline, pl_normalization_spline


"""Script-wide simulation and analysis settings.
"""
CFG_FILE_PATH = os.path.join(XIMPOL_CONFIG, 'herx1_pulsar11on.py')
OUT_FILE_PATH_BASE = os.path.join(XIMPOL_DATA, 'herx1_pulsar11on')
...
def plot(save=False):
    ...
    for j in range(len(E_BINNING)):
        ...
        if save:
            save_current_figure('herx1_11on_polarization_degree_%d' % j, OUTPUT_FOLDER, False)
        plt.figure('Polarization angle (%g-%g keV)' % (emin,emax))
        plt.title('Her X-1 (%g-%g keV)' % (emin,emax))
        pl_normalization_spline.plot(scale=60, onset=0, show=False,
        ...
        if save:
            save_current_figure('herx1_11on_polarization_angle_%d' %j, OUTPUT_FOLDER, False)
        ...
        if save:
            save_current_figure('herx1_11on_flux_%d' % j, OUTPUT_FOLDER, False)
        # plt.show()
