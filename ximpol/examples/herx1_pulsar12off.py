...
from ximpol.config.herx1_pulsar12off import pol_degree_spline, \
    pol_angle_spline, pl_normalization_spline


"""Script-wide simulation and analysis settings.
"""
CFG_FILE_PATH = os.path.join(XIMPOL_CONFIG, 'herx1_pulsar12off.py')
OUT_FILE_PATH_BASE = os.path.join(XIMPOL_DATA, 'herx1_pulsar12off')
...
def plot(save=False):
    ...
    for j in range(len(E_BINNING)):
        ...
        if save:
            save_current_figure('herx1_12off_polarization_degree_%d' % j, OUTPUT_FOLDER, False)
        ...
        if save:
            save_current_figure('herx1_12off_polarization_angle_%d' %j, OUTPUT_FOLDER, False)
        ...
        if save:
            save_current_figure('herx1_12off_flux_%d' % j, OUTPUT_FOLDER, False)
        # plt.show()
