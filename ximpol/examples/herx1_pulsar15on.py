...
from ximpol.config.herx1_pulsar15on import pol_degree_spline, \
    pol_angle_spline, pl_normalization_spline


"""Script-wide simulation and analysis settings.
"""
CFG_FILE_PATH = os.path.join(XIMPOL_CONFIG, 'herx1_pulsar15on.py')
OUT_FILE_PATH_BASE = os.path.join(XIMPOL_DATA, 'herx1_pulsar15on')
...
def plot(save=False):
    ...
    for j in range(len(E_BINNING)):
        ...
        if save:
            save_current_figure('herx1_15on_polarization_degree_%d' % j, OUTPUT_FOLDER, False)
        ...
        if save:
            save_current_figure('herx1_15on_polarization_angle_%d' %j, OUTPUT_FOLDER, False)
        ...
        if save:
            save_current_figure('herx1_15on_flux_%d' % j, OUTPUT_FOLDER, False)
        # plt.show()
