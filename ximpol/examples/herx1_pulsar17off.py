...
from ximpol.config.herx1_pulsar17off import pol_degree_spline, \
...
def plot(save=False):
    ...
    for j in range(len(E_BINNING)):
        ...
        if save:
            save_current_figure('herx1_17off_polarization_degree_%d' % j, OUTPUT_FOLDER, False)
        ...
        if save:
            save_current_figure('herx1_17off_polarization_angle_%d' %j, OUTPUT_FOLDER, False)
        ...
        if save:
            save_current_figure('herx1_17off_flux_%d' % j, OUTPUT_FOLDER, False)
        # plt.show()
