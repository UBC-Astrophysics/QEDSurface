...
OUT_FILE_PATH_BASE = os.path.join(XIMPOL_DATA, 'herx1_pulsar10on')
EVT_FILE_PATH = '%s.fits' % OUT_FILE_PATH_BASE
ANALYSIS_FILE_PATH = '%s_analysis_onebin.txt' % OUT_FILE_PATH_BASE
...
E_BINNING = np.linspace(2.0,8.0,60)
...
def plot(save=False):
    ...
    for j in range(len(E_BINNING)):
        ...
        if save:
            save_current_figure('herx1_10on_polarization_degree_%d' % j, OUTPUT_FOLDER, False)
        ...
        if save:
            save_current_figure('herx1_10on_polarization_angle_%d' %j, OUTPUT_FOLDER, False)
        ...
        if save:
            save_current_figure('herx1_10on_flux_%d' % j, OUTPUT_FOLDER, False)
        # plt.show()
